@testset "research_inverse.jl" begin
    ...
    result = CR_gammaReg_inv(topredict, [:d15min, :d45min, :d1h, :d6h], train, test)
    ...
    @test RMSD == gammaReg_inv("Duration", varnames, train, test)

    #The df is well sorted
    @test RMSD == minimum(result.RMSD)

    #The higher RMSD of the CR_gammaReg_inv corresponds to the higher found stepwise
    @test RMSD == gammaReg_inv("Duration", variables, train, test)
