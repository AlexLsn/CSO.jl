"""
    ...
    Tests all possible set of variables (limited to `nbVar` variables) fitting a gamma regression (log link).
...
function CR_gammaReg_inv(topredict::String, varnames::Vector{Symbol}, train::DataFrame, test::DataFrame, nbVar::Int64)
    ...
    for i in 1:length(model_var)
        vars = model_var[i]
        model = glm(Term(Symbol(topredict)) ~ sum(Term.(vars)), train, Gamma(), LogLink())
...
Selects the best set of explanatory variables after performing a forward feature selection with gamma regression (log link) selecting on the RMSD.
...
function stepRMSD_linreg(topredict::String, varnames::Vector{Symbol}, initial_list::Union{Vector{Symbol}, Vector{Any}}, train::DataFrame, test::DataFrame, verbose::Bool=true)
    ...
    while true
        ...
        for i=1:length(excluded)
            ...
            model = glm(Term(Symbol(topredict)) ~ sum(Term.(concat)), train, Gamma(), LogLink())
            predictions = convert(Vector{Float64},GLM.predict(model, test))
            RMSD = rmsd(predictions, test[:, Symbol(topredict)])
            push!(vec_RMSD, RMSD)
        end
        ...
        if included != []
            model = glm(Term(Symbol(topredict)) ~ sum(Term.(included)), train, Gamma(), LogLink())
            predictions = convert(Vector{Float64},GLM.predict(model, test))
            RMSD = rmsd(predictions, test[:, Symbol(topredict)])
            ex_RMSD = RMSD
        else
