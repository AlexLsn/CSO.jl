"""
    CR_gammaReg_inv(topredict::String, varnames::Vector{Symbol}, train::DataFrame, test::DataFrame)

Tests and ranks (RMSD) all possible set of variables fitting a gamma regression (inverse link).
...
"""
function CR_gammaReg_inv(topredict::String, varnames::Vector{Symbol}, train::DataFrame, test::DataFrame)
    ...
    for i in 1:length(model_var)
        vars = model_var[i]
        model = glm(Term(Symbol(topredict)) ~ sum(Term.(vars)), train, Gamma(), InverseLink())
...
"""
    stepRMSD_gammaReg_inv(topredict::String, varnames::Vector{Symbol}, initial_list::Vector{Symbol}, train::DataFrame, test::DataFrame, verbose::Bool=true)

Selects the best set of explanatory variables after performing a forward feature selection with gamma regression (inverse link) selecting on the RMSD.
...
function stepRMSD_gammaReg_inv(topredict::String, varnames::Vector{Symbol}, initial_list::Union{Vector{Symbol}, Vector{Any}}, train::DataFrame, test::DataFrame, verbose::Bool=true)
    ...
    while true
        ...
        for i=1:length(excluded)
            ...
            model = glm(Term(Symbol(topredict)) ~ sum(Term.(concat)), train, Gamma(), InverseLink())
            predictions = convert(Vector{Float64},GLM.predict(model, test))
            RMSD = rmsd(predictions, test[:, Symbol(topredict)])
            push!(vec_RMSD, RMSD)
        end
        ...
        if included != []
            model = glm(Term(Symbol(topredict)) ~ sum(Term.(included)), train, Gamma(), InverseLink())
            predictions = convert(Vector{Float64},GLM.predict(model, test))
            RMSD = rmsd(predictions, test[:, Symbol(topredict)])
            ex_RMSD = RMSD
        else
        ...
        if  best_RMSD < ex_RMSD
            best_feature = new_RMSD[new_RMSD.RMSD .== best_RMSD,:var][1] #[1] to extract a Symbol and not a Vector
