...
Performs a gamma regression (InverseLink) on a set of chosen variables with positive dataset.
...
function gammaReg_inv(topredict::String, varnames::Vector{Symbol}, train::DataFrame, test::DataFrame, RMSD::Bool=true)
    model = glm(Term(Symbol(topredict)) ~ sum(Term.(variables)), train, Gamma(), InverseLink())
